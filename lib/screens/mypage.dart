...
class _MyPageState extends State<MyPage> {
  ...
  final loc = widget.profile.currentLocation;

  // 1) currentLocation 에 값이 있으면 그걸 최우선으로 사용
  if (loc != null && loc.isValid) {
    return loc.address;
  }

  // 2) currentLocation 이 없지만 관심 지역이 있다면 첫 번째 관심 지역을 대체 값으로
  if (widget.profile.interestedAreas.isNotEmpty) {
    final mainArea = widget.profile.interestedAreas.first;

    String? reasonLabel;
    if (mainArea.customReason != null && mainArea.customReason!.isNotEmpty) {
      reasonLabel = mainArea.customReason;
    } else if (mainArea.reasons.isNotEmpty) {
      reasonLabel = mainArea.reasons.first; // 집 / 직장 / 기타 …
    }

    if (reasonLabel != null) {
      return '${mainArea.address} ($reasonLabel)';
    }
    return mainArea.address;
  }

  // 3) 아무 정보도 없을 때 기본 문구
  return '현재 위치 정보가 아직 등록되지 않았어요.';
}
